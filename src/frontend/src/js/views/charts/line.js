...
export const lineChartUpdate = () => {
	if (CHART_WRAPPER) {
		const updateColors = () => {
			lineChart.data.datasets[0].borderColor = rgba("--bs-highlight-rgb", 0.35);
			lineChart.data.datasets[0].hoverBackgroundColor = rgba("--bs-highlight-rgb", 0.35);
			lineChart.data.datasets[0].pointBorderColor = rgba("--bs-highlight-rgb", 0.35);
			lineChart.data.datasets[0].pointHoverBorderColor = rgba("--bs-highlight-rgb", 0.35);
			lineChart.data.datasets[0].pointHoverBackgroundColor = rgba("--bs-highlight-rgb", 0.35);

			lineChart.data.datasets[1].borderColor = rgba("--bs-highlight-rgb");
			lineChart.data.datasets[1].hoverBackgroundColor = rgba("--bs-highlight-rgb");
			lineChart.data.datasets[1].pointBorderColor = rgba("--bs-highlight-rgb");
			lineChart.data.datasets[1].pointHoverBorderColor = rgba("--bs-highlight-rgb");
			lineChart.data.datasets[1].pointHoverBackgroundColor = rgba("--bs-highlight-rgb");
		};

		setTimeout(() => {
			reloadChart(lineChart, updateColors());
		});
	}
};
