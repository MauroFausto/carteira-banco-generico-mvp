		...
		datasets: [
			{
				...
				pointBackgroundColor: rgba("--bs-highlight-rgb", 0.35),
			},
			{
				label: "February",
				data: SCATTER_DATA_2,
...
export const scatterChartUpdate = () => {
	if (CHART_WRAPPER) {
		const updateColors = () => {
			scatterChart.data.datasets[0].pointBackgroundColor = rgba("--bs-highlight-rgb", 0.35);
			scatterChart.data.datasets[1].pointBackgroundColor = rgba("--bs-highlight-rgb", 1);
		};

		setTimeout(() => {
			reloadChart(scatterChart, updateColors());
		});
	}
};
