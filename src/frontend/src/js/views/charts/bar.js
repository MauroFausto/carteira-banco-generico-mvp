...
export const barChartUpdate = () => {
	if (CHART_WRAPPER) {
		const updateColors = () => {
			barChart.data.datasets[0].backgroundColor = rgba("--bs-highlight-rgb", 0.35);
			barChart.data.datasets[1].backgroundColor = rgba("--bs-highlight-rgb");
		};

		setTimeout(() => {
			reloadChart(barChart, updateColors());
		});
	}
};
