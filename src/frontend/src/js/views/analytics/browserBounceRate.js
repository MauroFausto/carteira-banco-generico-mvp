...
export const browserBounceRate = () => {
	...
	if (WRAPPER) {
		DATA.map((item, index) => {
			list += `<div class="hstack gap-3">
                        <img class="h-7 w-7" src="${images[item.icon]}" alt="" />

                        <div class="flex-grow-1 hstack flex-wrap py-3 ${index !== last ? "border-bottom" : ""}">
                            ...
                            <div class="w-20 text-end d-none d-sm-block">
								<div class="badge rounded-pill fs-8 ms-auto text-body-emphasis d-inline-flex align-items-center mt-n1 bg-success bg-opacity-75 ${item.up ? "bg-success" : "bg-danger"}">
									${item.percentage}%
