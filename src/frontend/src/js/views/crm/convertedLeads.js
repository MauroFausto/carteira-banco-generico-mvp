...
export const convertedLeadsUpdate = () => {
	if (CHART_WRAPPER) {
		const updateColors = () => {
			convertedLeadsChart.data.datasets[0].backgroundColor = (context) => setThemeGradient(context, "main");
			convertedLeadsChart.data.datasets[1].backgroundColor = (context) => setThemeGradient(context, "sub");
			convertedLeadsChart.data.datasets[0].borderColor = rgba("--bs-highlight-rgb", 1);
			convertedLeadsChart.data.datasets[1].borderColor = rgba("--bs-highlight-secondary-rgb", 1);
		};

		setTimeout(() => {
			reloadChart(convertedLeadsChart, updateColors());
		});
	}
};
