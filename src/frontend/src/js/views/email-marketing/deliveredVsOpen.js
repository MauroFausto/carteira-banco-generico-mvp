	...
	const CHART_DATA = {
		...
		datasets: [
			...
			},
			{
				...
				borderRadius: 15,
				backgroundColor: rgba("--bs-highlight-secondary-rgb", 0.2),
				borderWidth: 0,
				maxBarThickness: 12,
			},
		],
...
export const deliveredVsOpenUpdate = () => {
	if (CHART_WRAPPER) {
		const updateColors = () => {
			deliveredVsOpenChart.data.datasets[0].backgroundColor = rgba("--bs-highlight-rgb", 0.9);
			deliveredVsOpenChart.data.datasets[1].backgroundColor = rgba("--bs-highlight-secondary-rgb", 0.2);
		};

		setTimeout(() => {
			reloadChart(deliveredVsOpenChart, updateColors());
		});
	}
};
