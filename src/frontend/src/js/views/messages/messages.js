...
export const messages = () => {
		...
		if (DM_WRAPPER) {
			DIRECT_MESSAGE_DATA.map((item, index) => {
				...
				if (item.img) {
					...
					dMavatar = `<div class="w-6 h-6 rounded-circle me-3 fs-7 fw-medium d-flex align-items-center justify-content-center text-uppercase bg-opacity-75 ${item.color}">${item.cap}</div>`;
				}
	...
	(() => {
		...
		if (WRAPPER) {
			...
			const MESSAGE_ACTIONS = `<div class="bg-active d-flex rounded position-absolute end-0 top-0 mt-n9 me-2 text-body-emphasis flex z-3 p-1 backdrop-blur-50">
		...
		if (FILES_WRAPPER) {
			CHANNEL_FILES.map((item) => {
				...
				if (item.img) {
					...
					icon = `<i class="ph rounded-circle w-8 h-8 me-3 fs-5 flex-shrink-0 d-grid place-content-center bg-active text-body ${item.icon}"></i>`;
				}
