		...
		FOLDERS_DATA.forEach((item) => {
			foldersList += `<button type="button" class="card card-hover px-4 py-3 text-start flex-row g-col-6 g-col-sm-4 g-col-md-3 g-col-lg-3 g-col-xxl-2">
                                <div class="flex-grow-1 overflow-hidden">
                                    <div class="text-truncate text-body-emphasis mb-0.5 w-100">${item.name}</div>
                                    <div class="text-body-secondary fs-8">${item.files} Files</div>
                                </div>
								<i class="ph ph-folder fs-4 d-none d-sm-block"></i>
                            </a>`;
		});
	...
	if (FILES_WRAPPER) {
		...
		FILES_DATA.forEach((item) => {
			if (item.type === "image") {
				...
				filePreview = `<div class="h-24 w-100 d-grid place-content-center rounded-1 bg-active"">
                                    <i class="w-10 h-10 rounded-circle d-grid place-content-center fs-3 ph ${item.icon} ${item.color}" bg-opacity-75></i>
                                </div> `;
			}
