...
export const photos = () => {
	...
	const RENDER_PHOTOS = () => {
		...
		DATA.map((group) => {
			...
			let photoGroupHeaderContent = `<div class="hstack gap-3 lh-1">
                                                <div class="text-body-emphasis">${group.time}</div>
                                                <div class="fs-7 text-body-secondary d-none d-sm-block mt-0.5">
                                                    ${group.location}
                                                    -
			...
			selectAllWrapper.classList.add("form-check", "mt-0", "ms-2", "me-3", CHECKBOX_TOGGLE_CLASS);
			...
			photoGroupHeader.prepend(selectAllWrapper);
			...
			photoGallery.classList.add("grid", "gap-3");
			...
			group.items.map((photo) => {
				...
				checkboxWrapper.classList.add("form-check", "position-absolute", "top-0", "start-0", "m-2");

				// Create and append the checkbox to select individual photo items
				let selectItem = document.createElement("input");
				selectItem.type = "checkbox";
				selectItem.checked = selectedPhotos.includes(photo.id);
				selectItem.classList.add(CHECKBOX_TOGGLE_CLASS, "photos-check", "form-check-input");
				selectItem.onclick = () => SELECT(selectItem, [photo.id]);
				checkboxWrapper.appendChild(selectItem);
				...
				photoItem.appendChild(checkboxWrapper);
